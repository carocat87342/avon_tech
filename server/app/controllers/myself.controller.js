...
const getForwardEmail = async (req, res) => {
  try {
    const dbResponse = await db.query(
      ...
      from users u where u.client_id=${req.client_id}
      and u.id<>$1 order by name limit 100`, [req.params.userId]
    );
...
const getLogins = async (req, res) => {
  try {
    const dbResponse = await db.query(
      `select dt, ip from user_login where user_id=$1 order by dt desc limit 20`, [req.params.userId]
    );
...
const getActivityHistory = async (req, res) => {
  try {
    const dbResponse = await db.query(
      ...
      where ul.user_id=$1 order by ul.dt desc limit 20`,
      [req.params.userId]
    );
